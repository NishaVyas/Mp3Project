<%-- 
    Document   : album
    Created on : 7 Sep, 2022, 12:43:58 AM
    Author     : Nisha
--%>
 ...
 %>
    ...
    <script>
        ...
        $(document).on("click", ".fa.fa-trash", function() {
                code = $(this).attr("rel");
                $.post(
                    "album_delete.jsp", {code: code}, function(data) {
               ...
               <div class="row-1">
                    <div class="col-sm-9">
                        <%   
                            ...
                            ResultSet rs1=st1.executeQuery("select * from admin_category where code='"+cat_code+"'");
                            while(rs1.next()){       
                       %>
                        
                       <h5>Category :<b style="color:blue"><%=rs1.getString("category")%></b></h5><br>
                        
                       <%
                             }
                         %>
                         <form  method="post" action="album_retrive.jsp?cat_code=<%=cat_code%>">
			       <label><b> Album Name: </label></b> <input type="text" name="album_name" class="form-control" required><br>
                          ...
                          <table class="table table-borderless">
                             <tbody>
                              ...
                              <%
                                 ...
                                 ResultSet rs=st.executeQuery("select * from album where cat_code='"+cat_code+"'");
                                 while (rs.next()){         
                               %>
                                <tr id="s<%=rs.getString("code")%>">	
                                        <td><img id="myBtn"  src="profile/<%=rs.getString("code")%>.jpg" style="width:40px;height:40px" class="img-circle"></td>
                                        <td id="t<%=rs.getString("code")%>" ><h5><%=rs.getString("album_name")%></h5></td>
                                        <td><a href="song.jsp?album_code=<%=rs.getString("code")%>" class="btn btn-primary">Add Song </a></td>
                                        <td><a href="upload.jsp?album_code=<%=rs.getString("code")%>&cat_code=<%=cat_code%>" class="fa fa-image" style="cursor:pointer"></a></td>
